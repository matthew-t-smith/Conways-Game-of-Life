...
public struct Grid {
    ...
    public subscript (pos: Position) -> CellState {
        get {
            let pos = normalize(position: pos, to: modulus); return _cells[pos.row][pos.col].state
        }
        set {
            let pos = normalize(position: pos, to: modulus); _cells[pos.row][pos.col].state = newValue
        }
    }
    ...
    public init(_ rows: Int, _ cols: Int, cellInitializer: (Position) -> CellState = { (arg) -> CellState in let (_, _) = arg; return .empty } ) {
...
public protocol GridProtocol: CustomStringConvertible {
    ...
    subscript (_: Position) -> CellState { get set }
    func next() -> Self
}
...
class StandardEngine: EngineProtocol {
    ...
    var size: Int = 10 {
        didSet {
            ...
            let name = Notification.Name(rawValue: "EngineSize")
    ...
    func step() -> Grid {
        let newGrid = grid.next()
        grid = newGrid
        //delegate?.engineDidUpdate(engine: EngineProtocol)
        let nc = NotificationCenter.default
